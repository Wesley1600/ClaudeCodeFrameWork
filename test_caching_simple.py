"""
Simple test script to verify core caching functionality (no PyTorch required).

This script tests the basic caching mechanisms without requiring PyTorch.
For full integration tests, run test_caching.py after installing dependencies.

Run with: python test_caching_simple.py
"""

import time
from operation_cache import (
    cached,
    clear_cache,
    get_cache_manager,
    print_cache_stats,
    CacheConfig,
    LRUCache,
)


def test_basic_caching():
    """Test basic caching functionality with a simple function."""
    print("\n" + "="*70)
    print("TEST 1: Basic Caching Functionality")
    print("="*70)

    call_count = [0]  # Use list to allow modification in nested function

    @cached(cache_type="memory", ttl=60)
    def expensive_computation(x: int, y: int) -> int:
        """Simulate an expensive computation."""
        call_count[0] += 1
        time.sleep(0.1)  # Simulate work
        return x + y

    # First call - should compute
    print("\n1. First call (should compute)...")
    start = time.time()
    result1 = expensive_computation(5, 10)
    time1 = time.time() - start
    print(f"   Result: {result1}, Time: {time1:.4f}s, Calls: {call_count[0]}")

    # Second call - should use cache
    print("\n2. Second call (should use cache)...")
    start = time.time()
    result2 = expensive_computation(5, 10)
    time2 = time.time() - start
    print(f"   Result: {result2}, Time: {time2:.4f}s, Calls: {call_count[0]}")

    # Different arguments - should compute
    print("\n3. Different arguments (should compute)...")
    start = time.time()
    result3 = expensive_computation(3, 7)
    time3 = time.time() - start
    print(f"   Result: {result3}, Time: {time3:.4f}s, Calls: {call_count[0]}")

    # Verify results
    assert result1 == 15, "First result incorrect"
    assert result2 == 15, "Second result incorrect"
    assert result3 == 10, "Third result incorrect"
    assert call_count[0] == 2, f"Expected 2 calls, got {call_count[0]}"
    assert time2 < time1 / 2, "Cache didn't speed up second call"

    print("\n✓ Basic caching test passed!")
    return True


def test_cache_stats():
    """Test cache statistics tracking."""
    print("\n" + "="*70)
    print("TEST 2: Cache Statistics")
    print("="*70)

    manager = get_cache_manager()

    # Clear cache first
    manager.memory_cache.stats.reset()

    @cached(cache_type="memory", ttl=60)
    def simple_func(x: int) -> int:
        return x * 2

    # Generate some cache hits and misses
    print("\n1. Generating cache activity...")
    simple_func(1)  # Miss
    simple_func(1)  # Hit
    simple_func(1)  # Hit
    simple_func(2)  # Miss
    simple_func(2)  # Hit

    stats = manager.memory_cache.get_stats()
    print(f"\n2. Statistics:")
    print(f"   Hits:     {stats['hits']}")
    print(f"   Misses:   {stats['misses']}")
    print(f"   Hit Rate: {stats['hit_rate']:.1%}")

    # Verify statistics
    assert stats['hits'] == 3, f"Expected 3 hits, got {stats['hits']}"
    assert stats['misses'] == 2, f"Expected 2 misses, got {stats['misses']}"
    assert abs(stats['hit_rate'] - 0.6) < 0.01, f"Expected 60% hit rate, got {stats['hit_rate']:.1%}"

    print("\n✓ Cache statistics test passed!")
    return True


def test_cache_eviction():
    """Test LRU cache eviction."""
    print("\n" + "="*70)
    print("TEST 3: LRU Cache Eviction")
    print("="*70)

    # Create a small cache
    cache = LRUCache(max_items=3, default_ttl=60)

    # Add items
    print("\n1. Adding 5 items to cache with max_items=3...")
    for i in range(5):
        cache.put(f"key_{i}", f"value_{i}")
        print(f"   Added key_{i}, cache size: {cache.size()}")

    # Check size
    size = cache.size()
    print(f"\n2. Final cache size: {size}")
    assert size == 3, f"Expected cache size 3, got {size}"

    # Check oldest items were evicted
    print("\n3. Checking eviction...")
    assert cache.get("key_0") is None, "key_0 should be evicted"
    assert cache.get("key_1") is None, "key_1 should be evicted"
    assert cache.get("key_2") is not None, "key_2 should exist"
    assert cache.get("key_3") is not None, "key_3 should exist"
    assert cache.get("key_4") is not None, "key_4 should exist"

    print("   ✓ Oldest items (key_0, key_1) were evicted")
    print("   ✓ Newest items (key_2, key_3, key_4) remain")

    print("\n✓ LRU eviction test passed!")
    return True


def test_disk_cache():
    """Test disk-based caching."""
    print("\n" + "="*70)
    print("TEST 4: Disk Cache")
    print("="*70)

    manager = get_cache_manager()

    # Clear disk cache first
    manager.embeddings_cache.clear()

    # Test disk cache operations
    print("\n1. Testing disk cache put/get...")
    test_data = {"numbers": [1, 2, 3, 4, 5], "message": "test"}

    # Put data
    manager.embeddings_cache.put("test_key", test_data)
    print("   ✓ Data written to disk")

    # Get data
    retrieved = manager.embeddings_cache.get("test_key")
    print("   ✓ Data retrieved from disk")

    # Verify
    assert retrieved == test_data, "Retrieved data doesn't match"
    print("   ✓ Data integrity verified")

    # Test non-existent key
    missing = manager.embeddings_cache.get("nonexistent_key")
    assert missing is None, "Should return None for missing key"
    print("   ✓ Missing key handling verified")

    print("\n✓ Disk cache test passed!")
    return True


def test_ttl_expiration():
    """Test TTL (time-to-live) expiration."""
    print("\n" + "="*70)
    print("TEST 5: TTL Expiration")
    print("="*70)

    cache = LRUCache(max_items=10, default_ttl=1)  # 1 second TTL

    # Add item
    print("\n1. Adding item with 1 second TTL...")
    cache.put("test_key", "test_value")

    # Get immediately
    value = cache.get("test_key")
    assert value == "test_value", "Should retrieve value immediately"
    print("   ✓ Value retrieved immediately")

    # Wait for expiration
    print("\n2. Waiting for expiration (1.5 seconds)...")
    time.sleep(1.5)

    # Try to get expired item
    expired_value = cache.get("test_key")
    assert expired_value is None, "Should return None for expired item"
    print("   ✓ Expired value correctly removed")

    print("\n✓ TTL expiration test passed!")
    return True


def test_multiple_cache_types():
    """Test different cache types."""
    print("\n" + "="*70)
    print("TEST 6: Multiple Cache Types")
    print("="*70)

    @cached(cache_type="memory", ttl=60, key_prefix="mem_")
    def memory_func(x: int) -> int:
        return x * 2

    @cached(cache_type="embeddings", ttl=60, key_prefix="emb_")
    def embeddings_func(x: int) -> int:
        return x * 3

    print("\n1. Testing memory cache...")
    result1 = memory_func(10)
    result1_cached = memory_func(10)
    assert result1 == result1_cached == 20
    print("   ✓ Memory cache working")

    print("\n2. Testing embeddings (disk) cache...")
    result2 = embeddings_func(10)
    result2_cached = embeddings_func(10)
    assert result2 == result2_cached == 30
    print("   ✓ Embeddings cache working")

    print("\n✓ Multiple cache types test passed!")
    return True


def run_all_tests():
    """Run all cache tests."""
    print("\n" + "="*70)
    print("OPERATION CACHE TEST SUITE (Simple)")
    print("="*70)

    # Clear all caches before testing
    print("\nClearing all caches before testing...")
    clear_cache()

    tests = [
        test_basic_caching,
        test_cache_stats,
        test_cache_eviction,
        test_disk_cache,
        test_ttl_expiration,
        test_multiple_cache_types,
    ]

    results = []
    for test in tests:
        try:
            result = test()
            results.append((test.__name__, result))
        except Exception as e:
            print(f"\n✗ Test {test.__name__} failed with error: {e}")
            import traceback
            traceback.print_exc()
            results.append((test.__name__, False))

    # Print summary
    print("\n" + "="*70)
    print("TEST SUMMARY")
    print("="*70)

    for name, passed in results:
        status = "✓ PASSED" if passed else "✗ FAILED"
        print(f"{status}: {name}")

    passed_count = sum(1 for _, p in results if p)
    total_count = len(results)

    print(f"\nTotal: {passed_count}/{total_count} tests passed")

    # Print final cache statistics
    print_cache_stats()

    return all(p for _, p in results)


if __name__ == "__main__":
    success = run_all_tests()

    if success:
        print("\n🎉 All tests passed! Caching system is working correctly.\n")
        print("Note: For full integration tests with PyTorch, run: python test_caching.py")
        print("      (requires: pip install -r requirements.txt)\n")
        exit(0)
    else:
        print("\n❌ Some tests failed. Please review the errors above.\n")
        exit(1)
